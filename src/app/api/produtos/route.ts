...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (produto.adicional)
      throw new Error(
        "Produtos adicionais possuem uma rota própria para inserção."
      );
...
export async function PATCH(req: NextRequest) {
  ...
  try {
    const validatedData = validateProdutoForm(await req.json());

    const result = await updateProduto(validatedData);
